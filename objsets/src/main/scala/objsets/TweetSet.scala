...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val moreRetweeted = filterMoreRetweeted
    if (!moreRetweeted.isEmpty)
      moreRetweeted.mostRetweeted
    else
      elem
  }

  def filterMoreRetweeted: TweetSet = {
    (left union right).filter((tweet: Tweet) => elem.retweets < tweet.retweets)
  }
...
object GoogleVsApple {
  ...
  def findTweets(allTweets: TweetSet, keywords: List[String]): TweetSet ={
    allTweets.filter((tweet: Tweet) => google.exists(keyword => tweet.text.contains(keyword)))
  }
