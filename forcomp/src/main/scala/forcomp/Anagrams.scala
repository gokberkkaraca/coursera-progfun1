...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toList.map(_.toLower).groupBy(identity).map(char => (char._1, char._2.length)).toList.sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences(s.foldLeft("")((word1, word2) => word1 ++ word2))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(Nil)
    case (char, int) :: tail =>
      for (x <- combinations(tail); n <- 0 to int)
        yield if (n == 0) x else (char, n) :: x
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    for (
      occurrence <- x
      if !ymap.contains(occurrence._1) || (occurrence._2 > ymap(occurrence._1))
    ) yield
      if (ymap.contains(occurrence._1)) (occurrence._1, occurrence._2 - ymap(occurrence._1))
      else occurrence
  }
